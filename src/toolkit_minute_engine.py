...
# ══════════════════════════════════════════════════════════════════════════════
# Position（复用 true_minute_engine 的 Position 类）
# ══════════════════════════════════════════════════════════════════════════════
...
class Position:
    ...
    def __init__(self, strat, product, code, opt_type, strike, open_price,
                 n, open_date, mult, expiry, mr, role, spot=0, exchange='',
                 group_id='', underlying_code=''):
        self.strat = strat
    ...
    def pnl_attribution(self, total_pnl=None, dt_days=1.0):
        ...
        g_pnl = sign * 0.5 * gamma_for_attr * ds * ds * self.mult * self.n if np.isfinite(gamma_for_attr) else 0.0
        # py_vollib的theta已经是日频值（dPrice/dt，t以天为单位），不需要再除365
        theta_for_attr = avg_greek(self.prev_theta, self.cur_theta)
        t_pnl = sign * theta_for_attr * float(dt_days) * self.mult * self.n if np.isfinite(theta_for_attr) else 0.0
        v_pnl = 0.0
        ...
        return {'delta_pnl': d_pnl, 'gamma_pnl': g_pnl,
                'theta_pnl': t_pnl, 'vega_pnl': v_pnl,
                'residual_pnl': residual_pnl, 'total_pnl': total}
...
class ToolkitMinuteEngine:
    ...
    def _output_results(self, nav_df, orders_df, stats, tag, elapsed):
        ...
        diagnostics_df = pd.DataFrame(self.diagnostics_records)
