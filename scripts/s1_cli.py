...
SCRIPT_REGISTRY = [
    ScriptEntry(
        "analyze-backtest",
        "analysis",
        "analysis/analyze_backtest_outputs.py",
        "通用回测输出分析，生成 NAV、回撤、保证金、Greeks、PnL attribution 图表。",
    ),
    ScriptEntry(
        "analyze-factors",
        "analysis",
        "analysis/analyze_factor_layers.py",
        "因子分层、Rank IC、Q1-Q5、相关性和残差 IC 分析。",
    ),
    ScriptEntry(
        "analyze-candidate-layers",
        "analysis",
        "analysis/analyze_candidate_universe_layers.py",
        "candidate universe 原始分层检查。",
    ),
    ScriptEntry(
        "analyze-candidate-corrected",
        "analysis",
        "analysis/analyze_candidate_universe_corrected.py",
        "修正口径后的 candidate universe / full shadow 因子检查。",
    ),
    ScriptEntry(
        "analyze-products",
        "analysis",
        "analysis/analyze_s1_product_suitability.py",
        "S1 品种适配度、流动性、尾部风险和长期可交易性分析。",
    ),
    ScriptEntry(
        "analyze-b6-products",
        "analysis",
        "analysis/analyze_b6_product_selection.py",
        "B6 品种筛选与残差 IC 相关分析。",
    ),
    ScriptEntry(
        "report-s1",
        "reports",
        "reports/build_s1_report_docx.py",
        "生成 S1 回测归因报告 Word / 飞书导入文档。",
    ),
    ScriptEntry(
        "report-factor-layer",
        "reports",
        "reports/build_factor_layer_report_docx.py",
        "生成因子分层检查 Word / 飞书导入文档。",
    ),
    ScriptEntry(
        "report-b4-formula",
        "reports",
        "reports/build_b4_formula_research_pack.py",
        "基于 Premium Pool 公式生成 B4 研究分析包。",
    ),
    ScriptEntry(
        "report-b6",
        "reports",
        "reports/build_b6_experiment_report.py",
        "生成 B6 实验报告。",
    ),
    ScriptEntry(
        "report-product-pool",
        "reports",
        "reports/build_product_pool_comparison_report.py",
        "生成 P3/P3B/品种池对比报告。",
    ),
    ScriptEntry(
        "report-stop-sweep",
        "reports",
        "reports/build_stop_loss_sweep_report.py",
        "生成止损倍数 sweep 对比报告。",
    ),
    ScriptEntry(
        "plot-candidate-extra",
        "reports",
        "reports/build_candidate_layer_extra_plots.py",
        "为因子报告补充额外图表。",
    ),
    ScriptEntry(
        "scorecard",
        "autoresearch",
        "autoresearch/s1_experiment_scorecard.py",
        "对单个回测结果生成核心绩效卡。",
    ),
    ScriptEntry(
        "audit",
        "autoresearch",
        "autoresearch/s1_experiment_audit.py",
        "对实验结果做审计，标记实现、数据和逻辑风险。",
    ),
    ScriptEntry(
        "autoresearch",
        "autoresearch",
        "autoresearch/s1_autoresearch_runner.py",
        "S1 自动研究队列执行器。",
    ),
]
